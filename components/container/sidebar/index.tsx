     ...
     {/* mobile sidebar start */}
      <div className="md:hidden w-full p-4 fixed top-0 flex items-center justify-between bg-slate-400">
        <div className="flex items-center gap-x-2">
          <Link
            href={"/"}
            className="text-xl font-bold tracking-wide italic cursor-pointer"
          >
            Connectify
          </Link>
          <Send className="rotate-12" />
        </div>
        <Button className="space-x-2" onClick={() => setIsNavopen(!isNavOpen)}>
          <span>Menu</span>
          <AlignJustify />
        </Button>
      </div>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-4/5 max-w-xs bg-white shadow-lg transform px-4 text-slate-600 ${
      ...
      >
        ...
        <div className="fixed shadow-lg w-[90%] bottom-2 ">
          <Button
            variant={"secondary"}
      ...
      {/* sidebar desktop */}
      <aside
        className={`hidden lg:flex realtive top-0 left-0 z-10 text-slate-600  min-h-screen w-1/4 flex-col overflow-y-hidden transition-transform duration-300 ease-in-out lg:static shadow-md shadow-slate-700 p-4 lg:translate-x-0`}
      >
        <div className="flex items-center gap-x-2 mx-auto mt-4">
          <Link
            href={"/"}
